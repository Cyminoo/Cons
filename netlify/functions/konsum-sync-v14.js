...
export default async (request) => {
  ...
  try {
    ...
    return json({
      ...
      message: "Cannot import @netlify/blobs. Ensure Netlify ran npm install (Git deploy or Clear cache & deploy).",
      details: String(err)
    }, { status: 500 });
  }
  ...
  async function getStateSafe(){
    ...
    try {
      return { state: JSON.parse(raw), corrupted: false };
    } catch {
  ...
  if (request.method === "GET") {
    const res = await getStateSafe();
    if (!res.state) {
      if (res.corrupted) return json({ error: "not_found", note: "corrupt_remote_cleared" }, { status: 404 });
      return bad("not found", 404);
    }
    return json(res.state, { status: 200 });
  }
  ...
  if (request.method === "PUT") {
    ...
    try {
      ...
      return json({ error: "invalid_json", received: raw.slice(0, 200) }, { status: 400 });
    }

    const incoming = sanitizeState(body);
    const { state: current } = await getStateSafe();
